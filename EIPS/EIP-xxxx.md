...
description: "Moves the computation and inclusion of the ExecutionPayload's state root for a block into the subsequent block, reducing latency and freeing resources on the critical path of block validation."
...
This change will lower latency at the tip, which will allow us to increase throughput and simplify the block production pipeline. It can also signficantly accellerate the timeline real-time ZK proving Ethereum.
...
3. ***Proactive Computation of Post-state Root*** (Implementation Recommendation):
...
3. **Simplify Timing for MEV-Boost and Builders**  
   - In a proposer-builder seperation (PBS) model, block construction speed is an important competitive dimension for builders and relays. 
...
1. **Light clients**  
   ...
   - Notably, many other blockchain networks already operate on a delayed state root and it has worked fine. One such network, Cosmos, is a multichain architeceture based entirely on light-client interoperability with a delayed state root.

2. **Reliant Contracts**  
   - We are not aware of any common contract types which assume availability of same-block state roots.  
...
Thanks to Vtalik Buterin, Sophia Gold, Tim Beiko, Kevaundry, ..., for feedback and input on this EIP. 
